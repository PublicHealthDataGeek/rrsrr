...
The previous chapter taught the basics of the R language.
...
The first exercise is to open up RStudio, take a look around the GUI and identify the main panes, shown in Figure \@ref(fig:rstudioui).
**Explore each of the main components of RStudio.**
Try clicking on different buttons in RStudio's GUI,  try changing the Global Settings (in the Tools menu) and see RStudio's short cuts by pressing `Alt-Shift-K` (or `Option+Shift+K` on Mac).
...
**Scripts** are files where R code is stored and you edit them in the Source Editor pane (the top left pane in Figure \@ref(fig:rstudioui)).
...
Code that you want to keep and share should be saved in R script files.  These are plain text files that have the `.R` extension.
...
To check that your project is saved, next close RStudio.???THIS SENTENCE DOESNT MAKE SENSE
In the next section we will re-open it and continue to edit the `chapter3.R` file.

## Writing and running code

Re-open RStudio and ensure that you have ?YOUR EMPTY CHAPTER£.RSCIPRT file open in the Source Editor.
...
This sends the code to the console.  When the code is sent to the console two objects are created, both of which are vectors of 5 elements (bonus: check their length using the `length()` function).
...
1. Click on the Run button on the toolbar to run all the code in a script.
1. Bonus: Use the function `source()` to run all the code in a script e.g. `source("chapter3.R")`
<!-- (but don't create an infinite loop!) -->

Don't forget you can jump between the console and the source editor by pressing `Ctl+1` and `Ctl+2`.
...
casualty_age = seq(from = 20, to = 60, by = 20) # This creates a sequences of numbers from 20 to 60 in increments of 20
set.seed(1) ? NEED EXPLANATION OF WHAT THIS IS DOING
dark = sample(x = c(TRUE, FALSE), size = 3, replace = TRUE) # ? NEED EXPLANATION OF WHAT THIS IS DOING
small_matrix = matrix(1:24, nrow = 12) ? NEED EXPLANATION OF WHAT THIS IS DOING
...
RStudio can help you write code by autocompleting it. RStudio will look for similar objects and functions after typing the first three letters of a name, for example:
...
You can view the help using `?` for example `?sum` and `?plot` in the console.
...
Typos are common and even experienced R users frequently see error messages as they undertake interactive data analysis.
...
See online articles such as "[23 RStudio Tips, Tricks and Shortcuts](https://www.dataquest.io/blog/rstudio-tips-tricks-shortcuts/)" for more comprehensive lists of useful RStudio shortcuts.
  ...
  - `Ctl+Enter` to send a line of code from the source editor JUST CHANGED TO SOURCE EDITOR AS THAT IS THE TERMINOLOGY YOU HAVE USED UP TO NOW to be executed or 'run' in the console. Amazingly, some other prominent IDEs such as Microsoft's VSCode editor, lack this important feature by default.
  - `Ctl+1` and `Ctl+2` to switch between the console (for writing test code and 'run once' commands) and the source editor (for writing code to keep).
  - `Alt+Up/Down` and `Alt+Shift+Up/Down` to move and copy lines of code up and down, handy when you want to re-order your code or ???make small changes to a copy of a line of code THIS DOESNT MAKE SENSE.
...
There is an active community of developers producing a range of extensions as well as RStudio itself is gradually evolving to meet the needs of 21^st^ Century data scientists.
